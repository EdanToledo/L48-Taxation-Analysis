...
def optimise_brackets(n_brackets):
    ...
    opt = BayesianOptimization(f=dynamic_f, domain=domain)
...
def optimise_n_brackets(tax_brackets):
    """
...
def optimise():
    """
    Runner function for BO over tax brackets.
    """

    """
    TODO:

    - The current situation is that we are passing in a massive config
      from the config.yaml file to bo_function.
